...
public class Test {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String filename = "D:\\北小洋\\21222\\面向对象程序设计\\OOP-Homework\\src\\com\\huawei\\classroom\\student\\h17\\rules.ini";
			BattleField
					.init("D:\\北小洋\\21222\\面向对象程序设计\\OOP-Homework\\src\\com\\huawei\\classroom\\student\\h17\\rules.ini");
			...
			for (int round = 0; round < 5; round++) {
				...
				String line = "第" + Integer.toString(round) + " 轮攻击";
				BattleField.results.add(line);
				//System.out.println("第" + round + " 轮攻击");
			}
			//测试结果见 reuslt.txt文件 供参考
			for (int i = 0; i < gameObjects.size(); i++) {
				String line = gameObjects.get(i).debug();
				BattleField.results.add(line);
//				System.out.println(gameObjects.get(i).debug());
			}
//			for(int i = 0; i < BattleField.results.size(); i++) {
//				System.out.println(BattleField.results.get(i));
//			}

		} catch (Exception e) {
			e.printStackTrace();
		}
		try (Reader reader = new FileReader("src/com/huawei/classroom/student/h17/result.txt");
			 LineNumberReader lineReader = new LineNumberReader(reader)){
				List<String> correct = new ArrayList<>();
				List<String> result = BattleField.results;
...
//				}
				if(correct.size() != result.size()) {
					System.out.println("NO! 数量都不一样啦！" + correct.size() +" " +  result.size());
				} else {
					boolean flag = true;
					for(int i = 0; i < result.size(); i++) {
						if(!result.get(i).equals(correct.get(i))) {
							System.out.println("NO! 做错啦！idx = " + i);
					...
					if(flag) {
						System.out.println("恭喜你做对啦！去做h18吧");
					}
				}
